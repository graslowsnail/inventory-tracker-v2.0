...
export default function PartHistoryCard() {
  ...
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {partHistorys.map((partHistory) => (
                <Link href={`/partHistory/${partHistory._id}`} passHref>
                <div
                    key={partHistory._id}
                ...
                >
                    <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-gray-900">Reset Date: {new Date(partHistory.resetDate).toLocaleString()}</p>
                                        <p className="text-sm text-gray-500">Different Parts Used: {partHistory.usedParts.length}</p>
                        <p className="text-sm text-gray-500">Total Items Used: {calculateBoxesUsed(partHistory.usedParts).toFixed(2)}</p>
